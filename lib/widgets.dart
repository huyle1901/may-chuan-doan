          ...
          title: ListTile(
            title: Column(
              ...
              children: <Widget>[
                ...
                Text(
                  '0x${characteristic.uuid.toString().substring(4, 8)}',
                  style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                        color: Theme.of(context).textTheme.bodySmall?.color,
                      ),
                ),
            ...
            children: <Widget>[
              ...
              IconButton(
                icon: Icon(Icons.file_upload,
                    color: Theme.of(context).iconTheme.color?.withOpacity(0.5)),
                onPressed: () async {
                  // Hiển thị dialog nhập giá trị
                  String? inputValue = await _showInputDialog(context);

                  // Nếu giá trị không rỗng và không null, gửi giá trị đi
                  if (inputValue != null && inputValue.isNotEmpty) {
                    // Chuyển giá trị string nhập thành giá trị byte
                    List<int> bytes = inputValue.codeUnits;
  ...
  Future<String?> _showInputDialog(BuildContext context) async {
    ...
    String inputText = await showDialog(
      ...
      builder: (BuildContext context) {
        String tempText = ''; // Biến tạm để lưu giá trị nhập vào

        return AlertDialog(
          title: Text('Nhập giá trị'),
          content: TextField(
            onChanged: (value) {
              tempText = value; // Cập nhật giá trị tạm khi người dùng thay đổi
            },
            ...
            TextButton(
              onPressed: () {
                Navigator.of(context)
                    .pop(); // Đóng hộp thoại khi người dùng nhấn Cancel
              },
              child: Text('Cancel'),
            ),
            TextButton(
              onPressed: () {
                Navigator.of(context).pop(
                    tempText); // Đóng hộp thoại và trả về giá trị tạm khi người dùng nhấn OK
              },
              child: Text('OK'),
            ),
          ],
...
String getNiceHexArray(List<int> bytes) {
  var hexString = bytes.map((i) => i.toRadixString(16).padLeft(2, '0')).join();
  return List.generate(
    hexString.length ~/ 2,
    (i) => String.fromCharCode(
        int.parse(hexString.substring(i * 2, (i * 2) + 2), radix: 16)),
